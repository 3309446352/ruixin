<template>
    <div class="new-address">
        <van-nav-bar :title="aid ? '编辑地址' : '新增地址'" left-text="返回" left-arrow fixed @click-left="back" />
        <br /><br />
        <van-address-edit show-postal show-set-default show-search-result :show-delete="!!aid"
            :area-columns-placeholder="['选择省', '选择市', '选择区']" :area-list="areaList" save-button-text="保存地址"
            :address-info="addressInfo" @save="save" @delete="removeAddress" />
    </div>
</template>
    ...
    methods: {
        ...
        save(content) {
            ...
            if (!this.aid) {
                ...
                // 编辑地址
                // 如果是编辑地址，则需要验证用户是否修改地址
                // let isModify = area.valid(o);
                let isModify = false;
